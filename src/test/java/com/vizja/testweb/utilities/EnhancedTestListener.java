...
public class EnhancedTestListener implements ITestListener, ISuiteListener {
    ...
    public void onTestStart(ITestResult result) {
        ...
        try {
            String suiteMapping = System.getProperty("test.suiteTestCaseMapping");
            if (suiteMapping != null && !suiteMapping.isEmpty()) {
                String classMethodKey = result.getTestClass().getName() + "#" + result.getMethod().getMethodName();
                for (String entry : suiteMapping.split(",")) {
                    int colon = entry.indexOf(':');
                    if (colon > 0 && classMethodKey.equals(entry.substring(0, colon))) {
                        String dbIdStr = entry.substring(colon + 1);
                        System.setProperty("test.testCaseDbId", dbIdStr);
                        System.out.println("🔗 [StepTracking] Resolved testCaseDbId from suite mapping: " + dbIdStr
                                + " for " + classMethodKey);
                        break;
                    }
                }
    ...
    public void onFinish(ITestContext context) {
        ...
        System.out
                .println("End Time: " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
        System.out.println("=".repeat(80));
        try {
